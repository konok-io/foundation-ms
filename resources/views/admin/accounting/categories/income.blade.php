...
@section('content')
@yield('breadcrumb', '<li class="breadcrumb-item"><a href="{{ route('admin.incomes.index') }}">Accounting</a></li><li class="breadcrumb-item active">Income Categories</li>')

<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0">Income Categories</h5>
        @can('income_categories.create')
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="btn-group btn-group-sm">
                                @can('income_categories.edit')
                                <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#editModal{{ $category->id }}">
                                    <i class="bi bi-pencil"></i>
                                </button>
                                <a href="{{ route('admin.income-categories.toggle', $category) }}" class="btn btn-outline-{{ $category->is_active ? 'warning' : 'success' }}">
                                    <i class="bi bi-power"></i>
                                </a>
                                @endcan
                                @can('income_categories.delete')
                                <form action="{{ route('admin.income-categories.destroy', $category) }}" method="POST" class="d-inline">
                                    @csrf @method('DELETE')
                    ...
                    <div class="modal fade" id="editModal{{ $category->id }}" tabindex="-1">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <form action="{{ route('admin.income-categories.update', $category) }}" method="POST">
                                    @csrf @method('PUT')
